...
vhost=wordpress
domain=infra.devopsgeekshubsacademy.click
BBDDHOST="infraestructura-production.cluster-cu8e3l8gnb4b.eu-west-1.rds.amazonaws.com"
...
#group = nginx
sudo sed -i 's/listen = 127.0.0.1:9000/;listen = 127.0.0.1:9000/g' "/etc/php-fpm.d/www.conf"
sudo sed -i '/;listen = 127.0.0.1:9000/a listen = /var/run/php-fpm/php-fpm.sock'  /etc/php-fpm.d/www.conf
...
sed -i "s/##HOST##/${BBDDHOST}/g" "/tmp/wp-config.php"
